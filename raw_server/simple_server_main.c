...
void * Thread_RS232_33f (void * num)
{
	...
	hUSB1 = openrs232("/dev/ttyUSB-robot",19200);
	//hUSB2 = openrs232("/dev/ttyUSB1",19200);
...
//	hUSB2 = openrs232("/dev/ttyUSB1",19200);
	...
	speed1=0;speed2=0;	
	SetRS232Motor33f_low_res(hUSB1,0x00,0x00);
	SetRS232MotorPID33f(hUSB1,0x00,0x00,80,45,0,360);
	
	//SetRS232Motor33f_low_res(hUSB2,0x00,0x00);
//	SetRS232MotorPID33f(hUSB2,0x00,0x00,80,45,0,360);
...
//		driveraw(hUSB2);		
		//pthread_mutex_unlock (& mutex_send);
		GetRS232Motor33f(hUSB1,&dataL,&dataR);
	//	GetRS232Motor33f(hUSB2,&dataL2,&dataR2);
...
void gestionnaire (int numero)
{
	switch (numero) {
		...
		case SIGINT :
			//fprintf (stdout, "\nSIGINT reçu\n"); fflush (stdout);
			StopMotorRS23233f(hUSB1);			
			close(hUSB1);
 			StopMotorRS23233f(hUSB2);                      
                        close(hUSB2);
			break;
	}
}
...
int openrs232(const char *device, int baudrate)
{
	...
	myhUSB = open(device, O_RDWR | O_NOCTTY);
	...
	tcsetattr(myhUSB, TCSANOW, &options);
