		...
		if (WaitForSingleObject(m_hClosed, 0) == WAIT_OBJECT_0)
		{
			...
			if (strlen(sName) != 0 && strlen(sName) < MAX_PATH - 10)
			{
				...
				if (nDataSize > 0)
				{
					...
					m_hSharedMemoryMutex[0] = CreateMutex(NULL, FALSE, (LPCWSTR)sMutex0);
					m_hSharedMemoryMutex[1] = CreateMutex(NULL, FALSE, (LPCWSTR)sMutex1);
								...
								if (m_pSize)
								{
									...
									if (bSharedMemorySizeOk)
									{
										...
										if (m_pSharedMemory[0] && m_pSharedMemory[1])
										{
											...
											m_hDataWrit[0]	= CreateEvent(NULL, FALSE, FALSE,	(LPCWSTR)sDataWrit0);
											m_hDataWrit[1]	= CreateEvent(NULL, FALSE, FALSE,	(LPCWSTR)sDataWrit1);
											m_hDataRead[0]	= CreateEvent(NULL, FALSE, TRUE,	(LPCWSTR)sDataRead0);
											m_hDataRead[1]	= CreateEvent(NULL, FALSE, TRUE,	(LPCWSTR)sDataRead1);
											if (m_hDataWrit[0] && m_hDataWrit[1] && m_hDataRead[0] && m_hDataRead[1])
											{
												m_hSecondInstanceAvailable = CreateEvent(NULL, FALSE, FALSE, (LPCWSTR)sName);
